...
if(isset($_POST['export_excel_analysis']))
{
		...
		for ($i = 1; $i <= 5; $i++) 
		{
			...
			$weighted=0.0;
                ...
                $query_total =  "SELECT COUNT(*) FROM `feedback_response`,`student_teacher` WHERE `feedback_response`.`username`=`student_teacher`.`username` AND `subject`='CN' AND `division`='A'";
                ...
                for ($j = 1; $j <= 5; $j++)   //marks
		{
    		$query="SELECT COUNT(*) FROM `feedback_response`, `student_teacher` WHERE `feedback_response`.`username`=`student_teacher`.`username` AND `".$q."` ='$j' AND `subject`='CN' AND `division`='A'";
